    ...
    <div className="bg-stone-900 rounded-xl border border-stone-800 p-4 flex flex-col items-center justify-center gap-2">
      ...
      <div className="relative w-32 h-32">
        <svg viewBox="0 0 120 120" className="w-full h-full">
          ...
          {['N','NE','L','SE','S','SO','O','NO'].map((d, i) => {
            ...
            return (
              <g key={d}>
                ...
                />
                <text x={xt} y={yt} textAnchor="middle" dominantBaseline="central"
                  fontSize={i % 2 === 0 ? '8' : '6'}
          ...
          >
            <polygon points="60,18 55,42 60,38 65,42" fill={color} opacity="0.95"/>
            <polygon points="60,102 55,78 60,82 65,78" fill="#44403c" opacity="0.6"/>
            <line x1="60" y1="22" x2="60" y2="98" stroke={color} strokeWidth="1" opacity="0.2"/>
          </g>
      ...
      <div className="text-center">
        <p className="text-3xl font-bold font-mono tabular-nums transition-colors duration-500"
          style={{ color }}>
          {isLoaded ? speed.toFixed(1) : '—'}
        </p>
        <p className="text-xs font-mono text-stone-500 mt-0.5">
          m/s · {directionLabel(direction)} · {direction}°
        </p>
      </div>
      ...
      <span className="text-[10px] font-mono text-stone-600">
        Open-Meteo · {cityName || `${latitude}, ${longitude}`}
      </span>
    </div>
  )
}